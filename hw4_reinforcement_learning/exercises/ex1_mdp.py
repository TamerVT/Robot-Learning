...
class PolicyIteration:
        ...
        while True:
            ...
            for s in range(self.env.n_states):
                ...
                for a in range(self.env.n_actions):
                    qsa = 0.0
                    # Compute Q(s, a) via the Bellman expectation equation
                    for prob, next_state, reward, done in self.env.P[s][a]:
                        qsa += prob * (reward + self.gamma * self.v[next_state] * (1.0 - float(done)))
                    qsa_list.append(qsa)

                # V^pi(s) = sum_a pi(a|s) * Q^pi(s, a)
                new_v[s] = sum(self.pi[s][a] * qsa_list[a] for a in range(self.env.n_actions))
                max_diff = max(max_diff, abs(new_v[s] - self.v[s]))

            self.v = new_v

            # Stop when the value function has converged (max change < theta)
    ...
    def policy_improvement(self):
        ...
        for s in range(self.env.n_states):
            ...
            for a in range(self.env.n_actions):
                qsa = 0.0
                # Compute Q(s, a) using the current value function
                for prob, next_state, reward, done in self.env.P[s][a]:
                    qsa += prob * (reward + self.gamma * self.v[next_state] * (1.0 - float(done)))
                qsa_list.append(qsa)
    ...
    def policy_iteration(self):
        ...
        while True:
            ...
            # greedily improve the policy w.r.t. the updated value function
...
class ValueIteration:
    ...
    def get_policy(self):
        ...
        for s in range(self.env.n_states):
            ...
            for a in range(self.env.n_actions):
                qsa = 0.0
                # Compute Q*(s, a) from the converged value function
                for prob, next_state, reward, done in self.env.P[s][a]:
                    qsa += prob * (reward + self.gamma * self.v[next_state] * (1.0 - float(done)))
                qsa_list.append(qsa)
